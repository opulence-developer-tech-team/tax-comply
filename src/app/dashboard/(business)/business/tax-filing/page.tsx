...
export default function BusinessTaxFilingPage() {
  ...
  if (!user) {
    throw new Error(
      "BusinessTaxFilingPage: User is required. This page should not render without authenticated user. " +
  ...
  if (!user.accountType) {
    throw new Error(
      "BusinessTaxFilingPage: user.accountType is required. User object must have accountType property. " +
  ...
  if (!validAccountTypes.includes(user.accountType)) {
    throw new Error(
      `BusinessTaxFilingPage: Invalid user.accountType "${user.accountType}". ` +
  ...
  if (!isBusinessAccount) {
    throw new Error(
      `BusinessTaxFilingPage: Invalid account type "${accountType}". Expected AccountType.Business.`
    );
  }
  ...
  if (!user._id) {
    throw new Error(
      "BusinessTaxFilingPage: user._id is required. User object must have _id property. " +
      "This is a critical data integrity error."
    );
  }
  ...
  if (isLoadingBusiness || (!hasFetchedBusiness) || (!selectedBusinessId && hasFetchedBusiness && businesses.length > 0)) {
    ...
    throw new Error(
      "BusinessTaxFilingPage: selectedBusinessId is required for Business accounts. " +
      "A business must be selected before accessing tax filing. " +
  ...
  if ((!entityId && isLoadingEntity) || isLoadingSubscription || !hasFetchedSubscription) {
    return (
      <RouteGuard requireAccountType={[AccountType.Business]} redirectTo="/dashboard/expenses" loadingMessage="Loading tax filing...">
  ...
  if (!currentSubscription) {
    throw new Error(
      "BusinessTaxFilingPage: currentSubscription is required. Subscription data must be fetched before rendering this page. " +
  ...
  if (!currentSubscription.plan) {
    throw new Error(
      "BusinessTaxFilingPage: currentSubscription.plan is required. Subscription object must have plan property. " +
      "This is a critical data integrity error."
    );
  }
  ...
  if (!validPlans.includes(currentSubscription.plan)) {
    throw new Error(
      `BusinessTaxFilingPage: Invalid subscription plan "${currentSubscription.plan}". ` +
  ...
  if (!entityId) {
    throw new Error("BusinessTaxFilingPage: entityId is required but is null.");
  }

  return (
    <RouteGuard requireAccountType={[AccountType.Business]} redirectTo="/dashboard/expenses" loadingMessage="Loading tax filing...">
      <TaxFilingContent accountId={accountId} entityId={entityId} accountType={AccountType.Business} currentPlan={currentPlan} />
    </RouteGuard>
  );
}
