...
import { PayrollContent } from "@/components/dashboard/payroll";
...
export default function BusinessPayrollPage() {
  ...
  if (!user) {
    throw new Error(
      "BusinessPayrollPage: User is required. This page should not render without authenticated user. " +
  ...
  if (!user.accountType) {
    throw new Error(
      "BusinessPayrollPage: user.accountType is required. User object must have accountType property. " +
  ...
  if (!validAccountTypes.includes(user.accountType)) {
    throw new Error(
      `BusinessPayrollPage: Invalid user.accountType "${user.accountType}". ` +
  ...
  if (!isBusinessAccount) {
    throw new Error(
      `BusinessPayrollPage: Invalid account type "${accountType}". Expected AccountType.Business.`
    );
  }
  ...
  if (isLoadingBusiness || (!hasFetchedBusiness) || (!selectedBusinessId && hasFetchedBusiness && businesses.length > 0)) {
    ...
    throw new Error(
      "BusinessPayrollPage: selectedBusinessId is required for Business accounts. " +
      "A business must be selected before accessing payroll management. " +
  ...
  if ((!entityId && isLoadingEntity) || isLoadingSubscription || !hasFetchedSubscription) {
    return (
      <RouteGuard requireAccountType={[AccountType.Business]} redirectTo="/dashboard/expenses" loadingMessage="Loading payroll management...">
  ...
  if (!currentSubscription) {
    throw new Error(
      "BusinessPayrollPage: currentSubscription is required. Subscription data must be fetched before rendering this page. " +
  ...
  if (!currentSubscription.plan) {
    throw new Error(
      "BusinessPayrollPage: currentSubscription.plan is required. Subscription object must have plan property. " +
      "This is a critical data integrity error."
    );
  }
  ...
  if (!validPlans.includes(currentSubscription.plan)) {
    throw new Error(
      `BusinessPayrollPage: Invalid subscription plan "${currentSubscription.plan}". ` +
  ...
  if (!entityId) {
    throw new Error("BusinessPayrollPage: entityId is required but is null.");
  }

  return (
    <RouteGuard requireAccountType={[AccountType.Business]} redirectTo="/dashboard/expenses" loadingMessage="Loading payroll management...">
      <PayrollContent entityId={entityId} accountType={AccountType.Business} currentPlan={currentPlan} />
    </RouteGuard>
  );
}
