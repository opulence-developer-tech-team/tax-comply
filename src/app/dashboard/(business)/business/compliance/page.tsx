...
export default function BusinessCompliancePage() {
  ...
  if (!user) {
    throw new Error(
      "BusinessCompliancePage: User is required. This page should not render without authenticated user. " +
  ...
  if (!user.accountType) {
    throw new Error(
      "BusinessCompliancePage: user.accountType is required. User object must have accountType property. " +
  ...
  if (!validAccountTypes.includes(user.accountType)) {
    throw new Error(
      `BusinessCompliancePage: Invalid user.accountType "${user.accountType}". ` +
  ...
  if (!isBusinessAccount) {
    throw new Error(
      `BusinessCompliancePage: Invalid account type "${accountType}". Expected AccountType.Business.`
    );
  }
  ...
  if (isLoadingBusiness || (!hasFetchedBusiness) || (!selectedBusinessId && hasFetchedBusiness && businesses.length > 0)) {
    ...
    throw new Error(
      "BusinessCompliancePage: selectedBusinessId is required for Business accounts. " +
      "A business must be selected before accessing compliance dashboard. " +
  ...
  if ((!entityId && isLoadingEntity) || isLoadingSubscription || !hasFetchedSubscription) {
    return (
      <RouteGuard requireAccountType={[AccountType.Business]} redirectTo="/dashboard/expenses" loadingMessage="Loading compliance dashboard...">
  ...
  if (!currentSubscription) {
    throw new Error(
      "BusinessCompliancePage: currentSubscription is required. Subscription data must be fetched before rendering this page. " +
  ...
  if (!currentSubscription.plan) {
    throw new Error(
      "BusinessCompliancePage: currentSubscription.plan is required. Subscription object must have plan property. " +
      "This is a critical data integrity error."
    );
  }
  ...
  if (!validPlans.includes(currentSubscription.plan)) {
    throw new Error(
      `BusinessCompliancePage: Invalid subscription plan "${currentSubscription.plan}". ` +
  ...
  if (!entityId) {
    throw new Error("BusinessCompliancePage: entityId is required but is null.");
  }

  return (
    <RouteGuard requireAccountType={[AccountType.Business]} redirectTo="/dashboard/expenses" loadingMessage="Loading compliance dashboard...">
      <ComplianceContent entityId={entityId} accountType={AccountType.Business} currentPlan={currentPlan} />
    </RouteGuard>
  );
}
