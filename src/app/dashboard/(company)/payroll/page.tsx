...
export default function CompanyPayrollPage() {
  ...
  if (!user) {
    throw new Error(
      "CompanyPayrollPage: User is required. This page should not render without authenticated user. " +
  ...
  if (!user.accountType) {
    throw new Error(
      "CompanyPayrollPage: user.accountType is required. User object must have accountType property. " +
  ...
  if (!validAccountTypes.includes(user.accountType)) {
    throw new Error(
      `CompanyPayrollPage: Invalid user.accountType "${user.accountType}". ` +
  ...
  if (!isCompanyAccount) {
    throw new Error(
      `CompanyPayrollPage: Invalid account type "${accountType}". Expected AccountType.Company.`
    );
  }
  ...
  if (isLoadingCompany || (!hasFetchedCompany) || (!selectedCompanyId && hasFetchedCompany && companies.length > 0)) {
    ...
    throw new Error(
      "CompanyPayrollPage: selectedCompanyId is required for Company accounts. " +
      "A company must be selected before accessing payroll management. " +
  ...
  if ((!entityId && isLoadingEntity) || isLoadingSubscription || !hasFetchedSubscription) {
    return (
      <RouteGuard requireAccountType={[AccountType.Company]} redirectTo="/dashboard/expenses" loadingMessage="Loading payroll management...">
  ...
  if (!currentSubscription) {
    throw new Error(
      "CompanyPayrollPage: currentSubscription is required. Subscription data must be fetched before rendering this page. " +
  ...
  if (!currentSubscription.plan) {
    throw new Error(
      "CompanyPayrollPage: currentSubscription.plan is required. Subscription object must have plan property. " +
      "This is a critical data integrity error."
    );
  }
  ...
  if (!validPlans.includes(currentSubscription.plan)) {
    throw new Error(
      `CompanyPayrollPage: Invalid subscription plan "${currentSubscription.plan}". ` +
  ...
  if (!entityId) {
    throw new Error("CompanyPayrollPage: entityId is required but is null.");
  }

  return (
    <RouteGuard requireAccountType={[AccountType.Company]} redirectTo="/dashboard/expenses" loadingMessage="Loading payroll management...">
      <PayrollContent entityId={entityId} accountType={AccountType.Company} currentPlan={currentPlan} />
    </RouteGuard>
  );
}
