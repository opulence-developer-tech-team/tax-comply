...
import { generatePAYERemittancePDF } from "@/lib/server/export/tax-filing-pdf";
...
async function handler(request: Request): Promise<NextResponse> {
  try {
    ...
    if (!accountTypeCheck.allowed) {
      logger.warn("Individual account attempted to generate PAYE remittance", {
    ...
    // Generate PDF
    // CRITICAL: Pass accountType to support both Company and Business accounts
    const pdfBuffer = await generatePAYERemittancePDF(entityId, month, year, accountType, isWatermarked);
    const monthNames = ["", "January", "February", "March", "April", "May", "June", 
      "July", "August", "September", "October", "November", "December"];
    const filename = `PAYE-Remittance-${monthNames[month]}-${year}.pdf`;
    ...
    logger.error("Error generating PAYE remittance PDF", error);
    return NextResponse.json(
      { 
        message: "error", 
        description: error.message || "An error occurred while generating the PAYE remittance document", 
        data: null 
      },
      { status: 500 }
    );
