...
import { generateVATReturnPDF, generateYearlyVATReturnPDF } from "@/lib/server/export/tax-filing-pdf";
...
async function handler(request: Request): Promise<NextResponse> {
  try {
    ...
    if (!accountTypeCheck.allowed) {
      logger.warn("Individual account attempted to generate VAT return", {
    ...
    const month = monthParam ? parseInt(monthParam) : 0; // 0 means yearly
    ...
    logger.error("Error generating VAT return PDF", error);
    return NextResponse.json(
      { 
        message: "error", 
        description: error.message || "An error occurred while generating the VAT return document", 
        data: null 
      },
      { status: 500 }
    );
