...
async function handler(request: Request): Promise<NextResponse> {
  try {
    ...
    const validation = businessValidator.onboarding(body);
    if (!validation.valid) {
      return validation.response!;
    }

    logger.info("Business creation attempt", {
      userId: auth.context.userId.toString(),
    });

    return await businessController.createBusiness(auth.context.userId, body);
  } catch (error: any) {
    const err = error instanceof Error ? error : new Error(String(error));
    logger.error("Unexpected error in business creation route", err);
    return new NextResponse(JSON.stringify({
      message: MessageResponse.Error,
