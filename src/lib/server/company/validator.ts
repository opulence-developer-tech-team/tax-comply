...
class CompanyValidator {
  public onboarding(body: ICompanyOnboarding) {
    const schema = Joi.object<ICompanyOnboarding>({
      ...
      cacNumber: Joi.string()
        ...
        .custom((value, helpers) => {
          // If CAC is provided, validate format
          if (value && value.trim() !== "") {
            if (!isValidCAC(value)) {
        ...
        .messages({
          "string.base": "CAC number must be text.",
          "string.max": "CAC number must not exceed 50 characters.",
          "string.invalid": "CAC number format is invalid. Expected format: RC123456, BN1234567, or IT123456",
        }),
      tin: Joi.string()
        .trim()
        .max(50)
      ...
      vatRegistrationNumber: Joi.string()
        ...
        .allow("")
        .custom((value, helpers) => {
          // If VRN is provided, validate format
          if (value && value.trim() !== "") {
            if (!isValidVRN(value)) {
              return helpers.error("string.invalid");
            }
      ...
      isCACRegistered: Joi.boolean().optional().messages({
        "boolean.base": "CAC registration status must be true or false.",
      }),
      ...
      phoneNumber: Joi.string()
        ...
        .allow("")
        .custom((value, helpers) => {
          // If phone number is provided, validate Nigerian format
          if (value && value.trim() !== "") {
      ...
      companyType: Joi.string().trim().max(100).optional().allow("").messages({
        "string.base": "Company type must be text.",
    ...
    // CRITICAL: If CAC registered, CAC number is required
    if (body.isCACRegistered === true) {
      const normalizedCAC = body.cacNumber ? normalizeCAC(body.cacNumber) : undefined;
      if (!normalizedCAC) {
        validationErrors.push(
          "CAC number is required if your company is registered with CAC."
        );
      } else if (!isValidCAC(normalizedCAC)) {
        validationErrors.push(
          "CAC number format is invalid. Expected format: RC123456, BN1234567, or IT123456"
        );
      }
    }
    ...
    if (validationErrors.length > 0) {
      return {
        ...
        response: utils.customResponse({
          ...
          description: validationErrors[0], // Return first error
          data: {
            errors: validationErrors, // Include all errors in data
          },
        }),
      };
    }
  ...
  public update(body: Partial<ICompanyOnboarding>) {
    const schema = Joi.object<Partial<ICompanyOnboarding>>({
    ...
    // CRITICAL: Additional conditional validations
    const validationErrors: string[] = [];
    ...
    // CRITICAL: If CAC registered, CAC number is required
    if (body.isCACRegistered === true && body.cacNumber !== undefined) {
      const normalizedCAC = body.cacNumber ? normalizeCAC(body.cacNumber) : undefined;
      if (!normalizedCAC) {
        validationErrors.push(
          "CAC number is required if your company is registered with CAC."
